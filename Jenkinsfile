pipeline {
    agent {
        docker {
            ...
            label 'docker'
            args '-u 1001:998 -v ${HOME}/.m2:${WORKSPACE}/.m2 -v ${HOME}/.docker:${WORKSPACE}/.docker -v /usr/bin/docker:/usr/bin/docker -v /var/run/docker.sock:/var/run/docker.sock'
        }
    }

    environment {
        DOCKER_REPOSITORY = 'docker.velovec.pro/inferno'
    ...
    post {
        always {
            sh 'docker rmi -f $(docker images | grep ${BUILD_VERSION} | awk \'{ print $3 }\')'
